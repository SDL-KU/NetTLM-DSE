...
LTreeNode* Segment_scheme::build_LTree_by_segmentation() {
    int num_s = 0;
    // LOG("5) build_LTree_by_segmentation");
    if (ltree) {
        delete ltree;
        ltree = nullptr;
    }
    ...
    for(lid_t i=0;i<=layer_num;++i){
        ...
        if(border.contains(i)){
            ...
            //LOG("NumChildren: " << Scut->get_children().size());
            l=i+1;
            layer_set.clear();
        }
    }
...
bool sa_accept(const SchNode::SchCost &best, const SchNode::SchCost &now, double T){
    ...
    double prob = exp((best.cost()-now.cost())/(0.25*T));
...
void Segment_scheme::SA_search_SPM(int iter_num, LTreeNode* root, int childno,bool base){
...
#endif
            place.mutate(Cluster::xlen*Cluster::ylen, 0, 0, 0, 0);
        }
        else{
    ...
    iter.rand_seed = rand_seed;
    best.rand_seed = rand_seed;
    ...
    uint best_idx;
    int cur_min_idx = 0, cur_max_idx = 0;
    int last_min_idx = 0, last_max_idx = 0;
    int sa_op;
    int delta;
    
    
    gemini_plus *gem_p = new gemini_plus();
    
    // iter.change_OA_DRAM();
    // iter.change_MK_row();
    // iter.spread_WE();
    std::cout << "Itreation Start" << std::endl;
    bool preRUN = false;
    bool firstTLM = true;
    ...
    static const auto t_iter_start = steady_clock::now();
    
    while(rnd<iter_num){

        place = iter;

        sa_op = place.mutate(std::max(1.0,dist), cur_min_idx, cur_max_idx, delta, sa_op);
        ...
        if (root->get_children().size() == 20)
        {
            static uint32_t cnt = 0;

            if (preRUN)
            {
                auto tmp_place = place.get_placement();
                gem_p->allDRAMs.clear();
                gem_p->assignedDpath.clear();
                
                cidx_t core_id = 0;
                for (auto pl : tmp_place)
                {
                    // LOG2("layer: " << pl.layerno << " / " << "part: " << pl.partno << " / core: " << core_id);
                    gem_p->assignedDpath[pl.layerno].push_back(core_id++);
                    
                }

                gem_p->data_dram.clear();
                for (auto dram : place.layer_DRAM)
                {
                    for (auto d_type : dram.second)
                    {
                        gem_p->data_dram[dram.first].push_back(d_type);
                    }
                }
                
                // std::cout << "gem_p->init()" << std::endl;
                gem_p->init();
                unsigned long long score = gem_p->get_perf_loop();

                now_cost.time = score;

                if(now_cost.time < 1302000)
                    preRUN = false;
            }
            ...
            if(!preRUN)
            {
                ...
                std::ofstream input("../../mapping_input.csv", std::ios::trunc);
                input << place << std::endl;
                input.close();
                input.open("mapping_tlm_sa.csv", std::ios::app);
                input << place << std::endl;
                input.close();

                int ret;
                ret = std::system("../../NetTLMSim-Fast");
                assert(ret == 0);

                std::ifstream output("../../tlm_output.txt");
                uint64_t x = 0;
                assert(output);
                while (output >> x);
                assert(x != 0);
                ...
                // LOG3(cnt << "," << x);
                output.close();
                assert(!std::remove("../../tlm_output.txt"));
                now_cost.time = x;

                if(x < 1302000 || firstTLM)
                {
                    ret = std::system("../../NetTLMSim-Full");
                    assert(ret == 0);
                    output.open("../../tlm_output.txt");
                    x = 0;
                    assert(output);
                    while (output >> x);
                    assert(x != 0);
                ...
                if (last_min_idx == cur_max_idx && last_max_idx == cur_min_idx)
                {
                    cur_min_idx = 0;
                    cur_max_idx = 0;
                }
                else
                {
                    ...
                    last_max_idx = cur_max_idx;
                }

                assert(output.eof());
                output.close();
                assert(!std::remove("../../tlm_log.txt"));

                // LOG_BY_J(rand_seed << "," << cnt << "," << best_cost.time, best);
        ...
        // }
        static uint32_t cnt = 0;
        delta = (int)now_cost.time - (int)iter_cost.time;
        // EDTIED END
        auto t_iter_now = steady_clock::now();
        auto t_dse_elap = t_iter_now - t_iter_start;
        auto t_dse_elap_minute = duration_cast<minutes>(t_dse_elap).count();
        if(sa_accept(iter_cost, now_cost, T)){
			iter = place;
            iter_cost = now_cost;
            LOG3(rnd << ": " << now_cost.time << ",1," << find_candidate << "," << t_dse_elap_minute);
            // LOG3(cnt++ << ": " << now_cost.energy << "," << now_cost.time << "," << delta << "," << 1 << "," << sa_op << "," << cur_min_idx << "," << cur_max_idx << "," << T << "," << preRUN << "," << find_candidate);
        } else {
            LOG3(rnd << ": " << now_cost.time << ",0," << find_candidate << "," << t_dse_elap_minute);
            // LOG3(cnt++ << ": " << now_cost.energy << "," << now_cost.time << "," << delta << "," << 0 << "," << sa_op << "," << cur_min_idx << "," << cur_max_idx << "," << T << "," << preRUN << "," << find_candidate);
        }

		if(iter_cost.cost() < best_cost.cost() && !preRUN && find_candidate){
			best_cost = iter_cost;
            best = iter;
            best_idx = rnd;
		}
        if (!preRUN)
        {
            ...
            LOG_HOUR(rand_seed << "," << cnt_tlm_run << "," << best_cost.time, best);
            LOG_MINUTE(rand_seed << "," << cnt_tlm_run << "," << best_cost.time, best);
            // if(rnd % 40 == 0 && rnd / 40 <= 6)  // 10 minutes
            // {
            //     std::string str_m = std::to_string(rnd / 40) + "m";
            //     std::string fperf = "log_perf_" + str_m + ".txt";
            //     std::string fmap = "log_dpoint_" + str_m + ".txt";
            //     std::string str_perf = std::to_string(rand_seed) + "," + std::to_string(rnd) + "," + std::to_string(best_cost.time);

            //     LOG_EXP(fperf, str_perf);
            //     LOG_EXP(fmap, best);
            // }

            // if(rnd % 240 == 0)  // 1 hours
            // {
            //     std::string str_h = std::to_string(rnd / 240) + "h";
            //     std::string fperf = "log_perf_" + str_h + ".txt";
            //     std::string fmap = "log_dpoint_" + str_h + ".txt";
            //     std::string str_perf = std::to_string(rand_seed) + "," + std::to_string(rnd) + "," + std::to_string(best_cost.time);
    ...
    LOG("best idx: " << best_idx);
    place=best;
    // for(auto &layer : place.layer_DRAM)
    // {
    //     std::string dramlist = "";
    //     for(auto &dram : layer.second)
    //         dramlist += std::to_string(dram) + " ";
        
    //     LOG3(layer.first << ": " << dramlist);
    // }
    layer_Dram = place.layer_DRAM;
...
len_t Segment_scheme::search_best_batch(lid_t start, lid_t end, SchNode::SchCost &result_cost,bool fast_mode) {
    ...
    //LOG(" 4) search_best_batch::border = " << border);
    batch[start] = 1;
    LTreeNode *segment;
    if (start != 0) {
        //LOG(" 4) search_best_batch::start = " << start);
        border.set(start-1);
        ...
        //LOG("ltree->get_children(): " << (uint8_t)segment->get_type());
        //LOG(" 4) search_best_batch::ltree = " << (uint8_t)ltree->get_type());
        //LOG(" 4) search_best_batch::segment = " << (uint8_t)segment->get_type());
        //LOG(" 4) search_best_batch::segment->get_children[0] = " << (uint8_t)segment->get_children()[0]->get_type());
    }
    ...
    for (len_t current_batch_size = 2; current_batch_size <= batch_num; ++current_batch_size) {
        if (batch_num % current_batch_size == 0) {
            ...
            builded = build_subSchTree(segment, {}, false, false, false, fast_mode);
            //LOG("size: " << (uint8_t)segment->get_children().size());
            if (!builded) {
                // LOG("Fail");
#ifdef DEBUG
            ...
            if (!never_valid) {
                ...
                if (best_cost.time == 0 || current_cost.cost() < best_cost.cost()) {
                    //LOG("best_batch_size: " << best_batch_size);
...
Segment_scheme* DP_search(int iter_num, bool base,bool fast_mode) {
    ...
    time_t start_time = time(nullptr);
    lid_t layer_num = network->len();
    //LOG("layer_num: " << layer_num);
    len_t current_batch;
    Bitset not_conv_layers, current_border;
    network->cal_not_conv(not_conv_layers);
    //LOG("not_conv_layer_num: " << not_conv_layers.count());
    // std::cout << "not_conv_layers: " << not_conv_layers << std::endl;
    Segment_scheme *iter = new Segment_scheme(layer_num, SchNode::tot_batch, not_conv_layers);
    // EDTIED Started
    Segment_scheme *custom_iter = new Segment_scheme(layer_num, SchNode::tot_batch, not_conv_layers);
    // EDITED End
    ...
    for (auto i = 0; i < layer_num; ++i) {
        ...
        // LOG(i << " iter->border: " << iter->border);
        // LOG(i << " dp_border: " << dp_border[i]);
    }
    ...
    // EDITED Start
    // std::cout << iter->border << std::endl;
    std::cout << "border: " << iter->border << std::endl;
    iter->border.reset(17);
    iter->border.reset(22);
    iter->border.reset(23);
    iter->border.reset(25);

    iter->border.set(19);
    iter->border.set(24);
    // cout << iter->border << endl;
    ...
    if(base){
        ...
        // EDITED start
        //custom_iter->copy_from(*iter);
        //custom_iter->build_SchTree(true);
        // EDITED end
        assert(builded);
        end_time = time(nullptr);
    ...
    // std::cout << "/////////////////////" << std::endl;
    // custom_iter->schtree->print_struct("\t");
    // std::cout << iter->placement[0] << std::endl;   // idx: layer group
    // std::cout << iter->placement[0].get_partition()[0].siz << std::endl;   // idx: layer group
    return iter;
}

bool build_custom_SchTree() {
    bool valid = true;
